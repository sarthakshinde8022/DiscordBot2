...
# ── Sync wrapper ──────────────────────────────────────────────────────
# All cogs use synchronous DB calls, so we run async asyncpg in a thread
...
class SyncConn:
    """Synchronous wrapper around asyncpg connection."""
    def __init__(self):
        self._conn = None
        self._rows = []

    def __enter__(self):
        self._conn = asyncio.run(asyncpg.connect(DATABASE_URL))
        return self
    ...
    def execute(self, sql, params=None):
        sql = _fix_sql(sql)
        if params:
            asyncio.run(self._conn.execute(sql, *params))
        else:
            asyncio.run(self._conn.execute(sql))
        return self

    def executemany(self, sql, params_list):
        sql = _fix_sql(sql)
        asyncio.run(self._conn.executemany(sql, params_list))

    def fetchone(self):
        row = asyncio.run(self._conn.fetchrow(self._last_sql, *self._last_params))
        if row is None:
            return None
        return FakeRow(dict(row))

    def fetchall(self):
        rows = asyncio.run(self._conn.fetch(self._last_sql, *self._last_params))
...
def _fix_sql(sql):
    ...
    for idx, part in enumerate(parts[:-1]):
        new_sql += part + f'${idx+1}'
    new_sql += parts[-1]
    return new_sql

# Simpler approach — use a proper connection class
class Connection:
...
class Cursor:
    ...
    def execute(self, sql, params=None):
        sql = _pg(sql)
        if params:
            self._result = self._run(self._conn.fetch(sql, *params))
        else:
            self._result = self._run(self._conn.fetch(sql))
        return self

    def executemany(self, sql, params_list):
        sql = _pg(sql)
        self._run(self._conn.executemany(sql, params_list))

    def fetchone(self):
        if self._result and len(self._result) > 0:
            return FakeRow(dict(self._result[0]))
        return None

    def fetchall(self):
        if self._result:
            return [FakeRow(dict(r)) for r in self._result]
        return []

def _pg(sql):
    """Convert ? or %s to $1, $2... for asyncpg."""
    # Replace %s style
    parts = sql.split('%s')
    if len(parts) > 1:
        new_sql = ''
...
def get_conn():
    """Get a synchronous-wrapped connection."""
    import threading
    result = []
    error = []
    def connect():
    ...
    try:
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS players (
                ...
                hon INTEGER DEFAULT 500, mudra INTEGER DEFAULT 0,
                medals INTEGER DEFAULT 0, omni_shards INTEGER DEFAULT 0,
                xp INTEGER DEFAULT 0, level INTEGER DEFAULT 1,
                selected_char INTEGER DEFAULT NULL,
                last_daily TEXT DEFAULT NULL, last_hourly TEXT DEFAULT NULL,
                boss_keys INTEGER DEFAULT 0, active_saga INTEGER DEFAULT NULL,
                craft_coins INTEGER DEFAULT 0, clan_id INTEGER DEFAULT NULL,
                clan_role TEXT DEFAULT NULL, created_at TEXT DEFAULT (now()::text)
            )
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS characters (
                id SERIAL PRIMARY KEY, name TEXT NOT NULL, rarity TEXT NOT NULL,
                element TEXT NOT NULL, base_hp INTEGER DEFAULT 1000,
                base_atk INTEGER DEFAULT 100, base_def INTEGER DEFAULT 80,
                description TEXT DEFAULT '', banner TEXT DEFAULT 'swarajya',
                move1_name TEXT DEFAULT 'Strike', move1_power INTEGER DEFAULT 100,
                move2_name TEXT DEFAULT 'Guard Break', move2_power INTEGER DEFAULT 80,
                move3_name TEXT DEFAULT 'Charge', move3_power INTEGER DEFAULT 120,
                move4_name TEXT DEFAULT 'Final Blow', move4_power INTEGER DEFAULT 150
            )
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS player_characters (
                id SERIAL PRIMARY KEY, user_id TEXT, char_id INTEGER,
                level INTEGER DEFAULT 1, tier INTEGER DEFAULT 0,
                xp INTEGER DEFAULT 0, is_favorite INTEGER DEFAULT 0,
                obtained_at TEXT DEFAULT (now()::text)
            )
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS battle_stats (
                user_id TEXT PRIMARY KEY, pvp_wins INTEGER DEFAULT 0,
                pvp_losses INTEGER DEFAULT 0, boss_wins INTEGER DEFAULT 0,
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS bosses (
                ...
                hp INTEGER DEFAULT 10000, atk INTEGER DEFAULT 600,
                def INTEGER DEFAULT 300, reward_hon INTEGER DEFAULT 500,
                reward_xp INTEGER DEFAULT 200, description TEXT DEFAULT '',
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS sagas (
                ...
                description TEXT DEFAULT '', unlock_level INTEGER DEFAULT 1
            )
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS missions (
                id SERIAL PRIMARY KEY, saga_id INTEGER NOT NULL,
                mission_num INTEGER NOT NULL, name TEXT NOT NULL,
                enemy_name TEXT NOT NULL, enemy_hp INTEGER DEFAULT 2000,
                enemy_atk INTEGER DEFAULT 150, enemy_def INTEGER DEFAULT 100,
                enemy_element TEXT DEFAULT 'Dark', reward_hon INTEGER DEFAULT 100,
                reward_xp INTEGER DEFAULT 50, reward_keys INTEGER DEFAULT 0,
                description TEXT DEFAULT ''
            )
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS player_missions (
                user_id TEXT, saga_id INTEGER, mission_num INTEGER,
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS tower_attempts (
                user_id TEXT PRIMARY KEY, last_attempt TEXT DEFAULT NULL
            )
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS clans (
                id SERIAL PRIMARY KEY, name TEXT UNIQUE NOT NULL,
                tag TEXT UNIQUE NOT NULL, description TEXT DEFAULT '',
                leader_id TEXT NOT NULL, level INTEGER DEFAULT 1,
                xp INTEGER DEFAULT 0, hon_bank INTEGER DEFAULT 0,
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS market (
                id SERIAL PRIMARY KEY, seller_id TEXT NOT NULL,
                item_type TEXT NOT NULL, item_ref_id INTEGER NOT NULL,
                price_hon INTEGER NOT NULL, listed_at TEXT DEFAULT (now()::text),
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS trades (
                ...
                receiver_id TEXT NOT NULL, sender_item_type TEXT,
                sender_item_ref INTEGER, sender_hon INTEGER DEFAULT 0,
                receiver_item_type TEXT, receiver_item_ref INTEGER,
                receiver_hon INTEGER DEFAULT 0, status TEXT DEFAULT 'pending',
                created_at TEXT DEFAULT (now()::text)
            )
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS items (
                ...
                rarity TEXT NOT NULL, atk_bonus INTEGER DEFAULT 0,
                def_bonus INTEGER DEFAULT 0, hp_bonus INTEGER DEFAULT 0,
                description TEXT DEFAULT '', cost_hon INTEGER DEFAULT 0
            )
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS player_items (
                id SERIAL PRIMARY KEY, user_id TEXT NOT NULL,
                item_id INTEGER NOT NULL, level INTEGER DEFAULT 1,
                is_equipped INTEGER DEFAULT 0, equipped_to INTEGER DEFAULT NULL,
                is_favorite INTEGER DEFAULT 0, obtained_at TEXT DEFAULT (now()::text)
            )
        """)
        await conn.execute("""
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS player_eggs (
                id SERIAL PRIMARY KEY, user_id TEXT NOT NULL,
                egg_type TEXT NOT NULL, hatch_time TEXT NOT NULL,
                hatched INTEGER DEFAULT 0, obtained_at TEXT DEFAULT (now()::text)
            )
        """)
        await conn.execute("""
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS player_pets (
                id SERIAL PRIMARY KEY, user_id TEXT NOT NULL,
                pet_id INTEGER NOT NULL, nickname TEXT DEFAULT NULL,
                is_active INTEGER DEFAULT 0, obtained_at TEXT DEFAULT (now()::text)
            )
        """)

        # Seeds
        count = await conn.fetchval("SELECT COUNT(*) FROM bosses")
        if count == 0:
            await _seed_bosses(conn)
        count = await conn.fetchval("SELECT COUNT(*) FROM characters")
        if count == 0:
            await _seed_characters(conn)
        count = await conn.fetchval("SELECT COUNT(*) FROM sagas")
        if count == 0:
            await _seed_sagas(conn)
            await _seed_missions(conn)
        count = await conn.fetchval("SELECT COUNT(*) FROM items")
        if count == 0:
            await _seed_items(conn)
        count = await conn.fetchval("SELECT COUNT(*) FROM pets")
        if count == 0:
            await _seed_pets(conn)

        print("✅ Database initialized.")
    finally:
        await conn.close()

async def _seed_bosses(conn):
    await conn.executemany(
        "INSERT INTO bosses (name,title,hp,atk,def,reward_hon,reward_xp,description,element) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)",
        [
...
async def _seed_sagas(conn):
    await conn.executemany(
        "INSERT INTO sagas (id,name,description,unlock_level) VALUES ($1,$2,$3,$4)",
        [
...
async def _seed_missions(conn):
    await conn.executemany(
        """INSERT INTO missions (saga_id,mission_num,name,enemy_name,enemy_hp,enemy_atk,enemy_def,enemy_element,reward_hon,reward_xp,reward_keys,description)
...
async def _seed_characters(conn):
    await conn.executemany(
        """INSERT INTO characters (name,rarity,element,base_hp,base_atk,base_def,description,banner,
           move1_name,move1_power,move2_name,move2_power,move3_name,move3_power,move4_name,move4_power)
...
async def _seed_items(conn):
    await conn.executemany(
        "INSERT INTO items (name,type,rarity,atk_bonus,def_bonus,hp_bonus,description,cost_hon) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)",
        [
...
async def _seed_pets(conn):
    await conn.executemany(
        "INSERT INTO pets (name,type,rarity,atk_bonus,def_bonus,hp_bonus,description,emoji) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)",
        [
