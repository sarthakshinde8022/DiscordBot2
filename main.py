...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
        ...
        await ctx.send(f"❌ Missing argument: `{error.param.name}`. Use `jay!help` for usage.")
    elif isinstance(error, commands.BadArgument):
        await ctx.send("❌ Invalid argument. Use `jay!help` for usage.")
    elif isinstance(error, commands.CommandInvokeError):
        original = error.original
        print(f"CommandInvokeError in {ctx.command}: {original}")
        await ctx.send(f"❌ Command error: `{type(original).__name__}: {original}`")
    else:
